function PokerSession(players, minBet, gameSpeed) {
  ...
  let gPlayerList = new LinkedList()
  ...
  let playerWrapper = function(player, balance) {
    return (function PlayerConstructor() {
      let tPlayer = player

      let tCards = []

      let tBalance = balance
  ...
  function nextBet(betSize) {
    if (gCurrent !== gPlayerList.length) gPlayerList[gCurrent++].makeBet(betSize)
    // else
    //
  }
