...
function Core() {
  ...
  async function start(ctx) {
    ...
    if (!user) db.insert(User.createDefault(id, first_name))
  }
    ...
    switch (ctx.update.callback_query.data) {
      ...
      case cfg.callbacks.game.jad:
        if (user === 0) {
          ...
          db.updateState(ctx.update.callback_query.from.id, cfg.states.JAD_BET_CHANCE)
        }
        break
  ...
  async function textHandler(ctx) {
    ...
    switch (user[cfg.user.state]) {
      ...
      case cfg.states.JAD_BET_CHANCE:
        ...
        db.updateBetChance(ctx.message.from.id, parseFloat(result[0]))
        await db.updateState(ctx.message.from.id, cfg.states.JAD_BET_SIZE)
        ctx.reply(locale.state.tellBetSize[lang], keyboards.jad.size(user[cfg.user.balance]))
        break
      case cfg.states.JAD_BET_SIZE:
        ...
        if (win === 0) {
          ...
          ctx.reply(
            locale.state.tellCongratulations(win, user[cfg.user.bet][cfg.user.chance])[
              user[cfg.user.language]
            ]
          )
          db.updateBalance(ctx.message.from.id, user[cfg.user.balance] + win)
        }
        db.updateState(ctx.message.from.id, cfg.states.EMPTY)
        break
    }
  }

  async function help(ctx) {
    ctx.reply(locale.help[(await db.getOne(ctx.message.from.id))[cfg.user.language]])
  }
  ...
  async function getBalance(ctx) {
    const user = await db.getOne(
      ctx.message ? ctx.message.from.id : ctx.update.callback_query.from.id
    )
  ...
  async function setBalance(ctx) {
    ...
    if (res.length !== 3) ctx.reply(locale.errors.dontUnderstand[user[cfg.user.language]])
