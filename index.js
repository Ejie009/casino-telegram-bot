...
bot.onText(/\/daun/, msg => {
    ...
    if(keyy) {
        ...
        bot.sendMessage(msg.chat.id,`Перезарядка!! Осталось еще ${String(24*60-cd)} м!`)
...
bot.onText(/\/sabju/, msg => {
    ...
    if(gt>=3600){ //+
        ...
        bot.sendMessage(msg.chat.id,`До следующего поступления сабжей осталось ${60-(datenow-dates_.fetch(msg.from.id))/60000} минут` )
    }
})
var bal_edit=false;
...
var allin=false;
    for(var i=0; i<players.length; i++){
        if(players[i].id==msg.from.id) {
            if(!msg.text.startsWith('!')) {bot.sendMessage(msg.chat.id, `${msg.from.first_name}, вы неправильно ввели число. Попробуйте еще раз (в начале ставьте "!").`)
                return}
            ...
            if(!players[i].state){
                ...
                console.log("Остаток баланса текущего игрока - "+(parseFloat(bal_arr.fetch(msg.from.id))-players[i].bet))
                    if(parseFloat(bal_arr.fetch(msg.from.id))<players[i].bet||players[i].bet==0){
                        ...
                        if(parseFloat(bal_arr.fetch(msg.from.id))==players[i].bet) {bal_arr.store(msg.from.id,'0')}
                        else{

                            bal_arr.store(msg.from.id,(parseFloat(bal_arr.fetch(msg.from.id))-players[i].bet))}}
                ...
                if(random_boolean){
                    ...
                    {
                        bot.sendMessage(msg.chat.id,`\u{274C}\u{274C}\u{274C}Пользователь ${msg.from.first_name}, проиграл ставку в размере ${players[i].bet} сабжей.\nТеперь его баланс составляет - ${bal_arr.fetch(msg.from.id)}! roflanpominki!`)
                    }
                    players.remove(i);
                                    }
